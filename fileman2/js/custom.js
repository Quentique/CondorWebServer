...
//  alert('"' + file.fullPath + "\" selected.\n To integrate with CKEditor or TinyMCE change INTEGRATION setting in conf.json. For more details see the Installation instructions at http://www.roxyfileman.com/install.");
  ...
  var closedd = RoxyUtils.GetUrlParam('close');
  var type = RoxyUtils.GetUrlParam('type');
  var url = RoxyUtils.GetUrlParam('url');
if ((type == "pdf" && file.ext == "pdf") || (type == "images" && file.ext !== "pdf"))  { 
	if (url == 'short') {
		$(window.parent.document).find('#'+fieldId).val(file.name);
	} else {
		$(window.parent.document).find('#'+fieldId).val("https://cvlcondorcet.fr/uploads/"+file.name);
	}
	if (closedd == "2"  ) {
		window.parent.closeCustomRoxy2();
	} else { 
		window.parent.closeCustomRoxy3(); 
	}
} else {
	alert('Un fichier ' + type + ' est demandé');
}
}
